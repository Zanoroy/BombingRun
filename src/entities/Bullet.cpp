...
Bullet::Bullet(float x, float y, float angle, float speed, void* owner, float size)
    : GameObject(x, y)
    , m_speed(speed)
    , m_lifetime(0.0f)
...
Bullet* Bullet::createWithTarget(float x, float y, float targetX, float targetY, float speed, void* owner, float size) {
    // Calculate angle to target
    float dx = targetX - x;
    float dy = targetY - y;
    float angle = std::atan2(dy, dx);
    
    return new Bullet(x, y, angle, speed, owner, size);
}
...
void Bullet::render(SDL_Renderer* renderer, TTF_Font* font) {
    ...
    if (m_customColor) {
        // Troop bullets: Black core with white trail (smaller)
        // Draw bullet core (black) - smaller size
        SDL_SetRenderDrawColor(renderer, m_colorR, m_colorG, m_colorB, 255);
        SDL_Rect bulletCore = {
            static_cast<int>(m_x - 1),
            static_cast<int>(m_y - 1),
            2,
            2
        };
        SDL_RenderFillRect(renderer, &bulletCore);
        
        // Draw white motion trail (smaller)
        SDL_SetRenderDrawColor(renderer, 255, 255, 255, 255);  // White trail
        for (int i = 1; i <= 4; i++) {
            float trailX = m_x - m_directionX * i * 1.5f;
            float trailY = m_y - m_directionY * i * 1.5f;
            SDL_RenderDrawPoint(renderer, trailX, trailY);
        }
    } else {
        // Fighter jet bullets: Original red bullets with glow
